...
data = np.loadtxt('data.txt', skiprows=5, usecols=(2, -2))
x, y = data[:, 0], data[:, 1]

# Calculate mean value for y after 0.2 ns
mean_value = np.mean(y[200:])
...
ax1.set_ylabel(r'density ($g/cm^3$)', fontsize=15)
ax1.tick_params(axis='both', labelsize=12)
ax1.yaxis.set_major_formatter(mtick.FuncFormatter(lambda x, pos: "${}$".format(mtick.ScalarFormatter(useOffset=False, useMathText=True)._formatSciNotation(f'{x:.10e}'))))
