...
def read_aims_output(infile1,options1):
    ...
    for i in np.arange(0,len(lines)):  # first pass to get loactions of data
        ...
        elif lines[i].strip()[0:44] == 'Running Born-Oppenheimer  molecular dynamics':
            performing_MD=True 
        elif lines[i].strip()[0:22] == 'Final atomic structure':
            start_final_structure_info=i
        elif lines[i].strip()[0:21]=='Total atomic forces (':
            start_forces=i
    ...
    for i in np.arange(start_control_info,start_control_info+22): #to get all important informations, charge, functional, k grid etc
        if len(lines[i].split())>2:
            if lines[i].split()[0].strip()=='Charge' and lines[i].split()[1].strip()=='=':
...
def read_aims_traj(infile1,options):
    ...
    if found_energy==False:
        print('No energy found!!!')
        print('Check if the following MD-step is written after last MD-step calculated.')
        print('Leads to problems, but could not figure out why this information is given somehow.')
        # exit-command is commented due to issue above
        #exit()
